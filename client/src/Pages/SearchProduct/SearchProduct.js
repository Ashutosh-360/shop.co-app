...
function SearchProduct() {
  ...
  useEffect(() => {
    GetData("get_products", {}, getProductsHandler);
    GetData("get_product_filters", {}, getFiltersDataHandler);
  }, []);

  const getProductsHandler = (res) => {
    setProductsData([...res?.data?.results]);
    setIsLoading(false);
  };

  const getFiltersDataHandler = (res) => {
    setFiltersData(res.data.results);
    setIsLoading(false);
  };
        ...
        <div className="flex gap-3 py-8 px-1 max-w-screen-xl m-auto  min-h-[100vh]">
          {/* <div className={`h-fit border rounded-lg ${style.filterContainer}`}>
            ...
            <div className="flex flex-col gap-2 p-4">
              {filtersData?.categories?.map((item) => {
                return (
            ...
            <div className="flex flex-col gap-2 p-4">
              ...
              <div className="flex gap-2 flex-wrap ">
                {filtersData?.size?.map((item) => {
                  return (
                    <div
                      className={`rounded-full p-1 cursor-pointer px-5 bg-gray-200 transition-all border ${
            ...
            <div className="flex justify-center mb-4">
              <button className="rounded-3xl bg-black text-white py-2 px-8">
                Apply Filters
              </button>
            </div>
          </div> */}
